...
async function run() {
  try {
    ...
    // use verifyAdmin middleware after verifyToken 
    ...
    app.delete('/menu/:id', verifyToken, verifyAdmin, async (req, res) => {
      const id = req.params.id;
      let filter = { _id: id };
      let result = await menuCollection.deleteOne(filter);

      if (result.deletedCount < 1) {
        filter = { _id: new ObjectId(id) };
        result = await menuCollection.deleteOne(filter)
      }
      res.send(result)
    })
    ...
    app.get('/menu/:id', async (req, res) => {
      ...
      if (!result) {
        filter = { _id: new ObjectId(id) };
        result = await menuCollection.findOne(filter)
      }
      res.send(result)
    })
